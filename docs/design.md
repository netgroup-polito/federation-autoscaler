...
| **Random** | None (pure random) | The Broker picks a random provider with available capacity — no ranking metric, no sticky cache. The simplest policy: useful when the consumer has no preference and wants to minimise broker computation. Also used as the baseline (Phase A) in comparative experiments. Random placements never migrate (no metric to compare). |
| **ConsumerChoice** | Delegated to a local LLM | The Broker returns **all** available providers with no masking; the Consumer Agent passes the full list, the user's natural-language request (`ConsumerPolicySpec.UserPrompt`) and the consumer's own discovered location to a local LLM (e.g. Ollama), which interprets them and selects a single provider. The LLM gets raw data only (coordinates, carbon intensity, cost, capacity) in plain JSON mode with the server's default sampling; nothing, distance included, is precomputed on its behalf. The prompt explains each field (unit, which end is better — a farther provider included — and which request words refer to it), and gives a trigonometry-free rule for comparing coordinates when the request is about proximity and has the model copy, for every provider, the values the request is about before it ranks. The request takes priority: the prompt's cost-first defaults apply only when the request states no preference or to break ties. Falls back deterministically (lowest cost, then lowest carbon intensity, then most free chunks, among providers with capacity) if the AI is unreachable, exceeds `--ollama-timeout` (default 120 s), returns an invalid choice, or no `--ollama-url` is configured. Enabled per consumer with `consumer-up.sh --ollama-url` (Ansible: `fa_ollama_url`), which fills `agent-config`. |
